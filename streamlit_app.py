...
# ==================== MODULE: INVENTORY ====================
    ...
    st.title("🏷️ Inventory & Restocking Hub")
    ...
    with t2:
        ...
        if low_stock_items:
            ...
            with col_b1:
                st.warning(f"⚠️ **{len(low_stock_items)} products** require restocking to maintain safety buffers.")
            with col_b2:
            ...
            for p in low_stock_items:
                ...
                with st.expander(f"{'🔴 [OUT OF STOCK]' if p['stock'] <= 0 else '🟡 [LOW STOCK]'} {p['name']} ({p['sku']}) — Current: {p['stock']} units | Target Min: {p['min_stock']} units", expanded=True):
                    ...
                    with c3:
                        ...
                        s_choice = st.selectbox("Supplier", [None] + [s['id'] for s in suppliers], format_func=lambda x: "Direct Supplier" if x is None else next(s['name'] for s in suppliers if s['id'] == x), key=f"rec_sup_{p['id']}")
                        if st.button(f"📥 Restock {p['name']}", key=f"btn_restock_{p['id']}", type="primary"):
                            ...
                            conn.commit()
                            conn.close()
                            st.success(f"Restocked {q_input} units of {p['name']}! PO #{po_num} generated.")
                            st.rerun()
        else:
    ...
    with t3:
        ...
        if products:
            with st.form("quick_stock_adjust_form", clear_on_submit=True):
                p_adj_id = st.selectbox("Select Product", [p['id'] for p in products], format_func=lambda x: f"{next(p['name'] for p in products if p['id'] == x)} (SKU: {next(p['sku'] for p in products if p['id'] == x)}, Current Stock: {next(p['stock'] for p in products if p['id'] == x)})")
...
# ==================== MODULE: PURCHASES ====================
    ...
    t1, t2, t3 = st.tabs(["📋 Purchase History", "📥 Receive Single Stock Shipment", "⚡ Multi-Item Supplier Restock"])
    ...
    with t3:
        ...
        if not products:
            ...
            m_supp_id = st.selectbox("Select Supplier for PO", [s['id'] for s in suppliers] if suppliers else [None], format_func=lambda x: "Direct Purchase / General Vendor" if not suppliers or x is None else next(s['name'] for s in suppliers if s['id'] == x), key="multi_restock_supp")

            selected_products = st.multiselect("Select Products to Restock in this PO", [p['id'] for p in products], format_func=lambda x: f"{next(p['name'] for p in products if p['id'] == x)} (Stock: {next(p['stock'] for p in products if p['id'] == x)}, Buy: ₹{next(p['purchase_price'] for p in products if p['id'] == x):,.2f})", key="multi_restock_prods")
            ...
            if selected_products:
                ...
                if st.button("🚀 Confirm & Issue Multi-Item PO", type="primary", use_container_width=True):
                    ...
                    conn.commit()
                    conn.close()
                    st.success(f"Batch Purchase Order #{po_batch_no} created successfully for ₹{est_total:,.2f}! Warehouse stock updated.")
                    st.rerun()
